...
def parse_dms(coor: str) -> float:
    ...
    '''
    # Usando expressões regulares para dividir a string de coordenadas em partes.
    parts = re.split('[^\d\w]+', coor)

    # Convertendo as partes da string em graus, minutos e segundos.
    degrees = float(parts[0])  # Graus são convertidos diretamente para float.
    minutes = float(parts[1])  # Minutos são convertidos para float.
    seconds = float(parts[2]+'.'+parts[3])  # Segundos e fração de segundos são concatenados e convertidos.

    # A última parte da string DMS indica a direção (N, S, E, W).
    direction = parts[4]  # Direção é simplesmente atribuída como string.

    # Calculando a coordenada decimal.
    dec_coord = degrees + minutes / 60 + seconds / 3600  # Conversão de minutos e segundos para graus.

    # Se a direção for Sul ou Oeste, o valor é negativo.
    if direction in ['S', 'W']:
        dec_coord *= -1  # Multiplicando por -1 para converter a coordenada para negativa.

    # Retornando o valor decimal da coordenada.
    return dec_coord
...
#-----Inicio Solução: Visualiação de dados ausentes
def visualize_missing_values_estimation(df: pd.core.frame.DataFrame, day: datetime):
    '''
    Visualiza duas formas de interpolar os dados: vizinho mais próximo e último valor,
    e os compara com os dados reais.
    
    Argumentos:
        df (pd.core.frame.DataFrame): O dataframe.
        day (datetime): O dia escolhido para plotar.
    '''
    day = day.date()

    # Filtra os dados para o dia especificado para a estação USM
    ...
    def draw(sample, station, missing_index, target):
        # Cria uma cópia dos dados para manipulação
        sample = sample.copy()
        # Adiciona um discriminador de tempo para auxiliar na interpolação
        sample.insert(
            0,
            'time_discriminator', 
        ...
        # Dados reais para a estação especificada
        real = sample[sample['Station'] == station]
        example1 = real.copy()
        real = real.reset_index()
        example1 = example1.reset_index()
        # Define alguns valores como NaN para simular valores ausentes
        example1.loc[missing_index, target] = float('NaN')

        # Índices dos dados ausentes e seus vizinhos
        ...
        plt.plot(missing_before_after, real.loc[missing_before_after][target], 'r--o', label='valores reais')

        # Prepara os dados para interpolação
        ...
        # Interpolação com vizinho mais próximo
        example1[f'new{target}'] = imputer.transform(example1[['time_discriminator', 'Latitude', 'Longitude', target]])[:,3]
        plt.plot(missing_before_after, example1.loc[missing_before_after][f'new{target}'], 'g--o', label='vizinho mais próximo')

        plt.plot(example1.index, example1[target], '-*')

        # Alteração realizada aqui: substituído `fillna(method='ffill')` por `ffill()`
        example1[f'ffill{target}'] = example1[target].ffill()
        # Interpolação com o último valor conhecido
        plt.plot(missing_before_after, example1.loc[missing_before_after][f'ffill{target}'], 'y--*', label='último valor conhecido')

        # Configurações adicionais da plotagem
        plt.xlabel('Hora do dia', fontsize=FONT_SIZE_AXES)
        plt.ylabel(f'Concentração de {target}', fontsize=FONT_SIZE_AXES)
        plt.title('Estimando valores ausentes', fontsize=FONT_SIZE_TITLE)
        plt.legend(loc="upper left", fontsize=FONT_SIZE_TICKS)
        plt.xticks(fontsize=FONT_SIZE_TICKS)
        plt.yticks(fontsize=FONT_SIZE_TICKS)
        plt.show()
    
    # Seletor de parâmetros para visualização
    ...
    # Widgets para seleção dos parâmetros
    station_selection = widgets.Dropdown(
        options=df.Station.unique(),
        description='Estação',
        value='USM'
    ...
    # Retorna a interação com os widgets
    return interact(
        selector,
        station=station_selection,
...
#-----FIM Solução: Visualiação de dados ausentes

#-----Início Calculando MAE e estabelecer uma linha base
def calculate_mae_for_nearest_station(df: pd.core.frame.DataFrame, target: str) -> Dict[str, float]:
    '''
    Cria um modelo de vizinho mais próximo e executa nos seus dados de teste para calcular o MAE.
    
    Argumentos:
        df (pd.core.frame.DataFrame): O DataFrame com os dados.
        target (str): O poluente escolhido para o qual deseja calcular o MAE.
    '''
    # Remove as linhas com valores ausentes para criar um DataFrame "limpo" para treino.
    df2 = df.dropna(inplace=False)
    
    # Insere uma coluna para discriminar o tempo, facilitando a imputação baseada no tempo.
    ...
    # Cria e ajusta o imputador KNN usando apenas o conjunto de treino.
    imputer = KNNImputer(n_neighbors=1)
    imputer.fit(train_df[['time_discriminator','Latitude', 'Longitude', target]])

    # Prepara os dados de teste e substitui o valor alvo por NaN para simular valores ausentes.
    y_test = test_df[target].values
    test_df2 = test_df.copy()
    test_df2.loc[test_df.index, target] = float("NAN")

    # Usa o imputador ajustado para prever os valores ausentes.
    y_pred = imputer.transform(test_df2[['time_discriminator', 'Latitude', 'Longitude', target]])[:,3]    
    return {"MAE": mean_absolute_error(y_pred, y_test)}   # Retorna um dicionário com o MAE calculado.

#-----FIM Calculando MAE e estabelecer uma linha base


#-----Início do treino de rede neural
def build_keras_model(input_size: int) -> tf.keras.Model:
    '''
    Constrói uma rede neural com três camadas totalmente conectadas (tamanhos: 64, 32, 1)
    
    Argumentos:
        input_size (int): O tamanho da entrada
        
    Retorna:
        model (tf.keras.Model): A rede neural
    '''
    model = keras.Sequential([
        layers.Dense(64, activation='relu', input_shape=[input_size]),  # Camada com 64 neurônios e ativação ReLU
        layers.Dense(32, activation='relu'),  # Camada subsequente com 32 neurônios e ativação ReLU
        layers.Dense(1)  # Camada de saída com 1 neurônio para regressão
      ])

    optimizer = tf.keras.optimizers.RMSprop(0.007)  # Otimizador RMSprop com taxa de aprendizado de 0.007

    model.compile(loss='mse',  # Usa o erro quadrático médio como função de perda
                optimizer=optimizer,  # Define o otimizador
                metrics=['mae'])  # Monitora o erro médio absoluto como métrica
    
    return model  # Retorna o modelo compilado

#-----FIM do treino de rede neural
...
def create_plot_with_preditions(
    ...
    def plot_predictions(station, size, start_index):
        try:
            # Garanta que as condições sejam aplicadas corretamente, evitando reindexações indesejadas
            data = df[(df['DateTime'] > start_date) & (df['DateTime'] < end_date) & (df['Station'] == station)]
    
            # Seleciona as características e o alvo já filtrados anteriormente
            X_test = data[feature_names]
            y_test = data[target]
    
            # Normaliza as características de teste
